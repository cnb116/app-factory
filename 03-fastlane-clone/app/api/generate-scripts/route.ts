...
[자막 줄바꿈 규칙 — 반드시 지킬 것]
hookLine, painAgitation, cta 이 세 항목은 실제로 화자가 말하는 대사이므로, Vrew 같은 자막 편집 프로그램에 그대로 붙여넣었을 때 추가 수정 없이 바로 쓸 수 있어야 한다. 그러기 위해 아래 규칙을 지켜서 작성한다:
- 한 줄은 공백 포함 15~18자를 넘지 않는다.
- 문장이 길면 자연스러운 구 단위로 끊어서 여러 줄로 나누고, 줄바꿈은 문자열 안에 \n으로 표시한다.
- 각 줄만 따로 읽어도 자막으로서 어색하지 않아야 한다.
예시: "저 작년에\\n10kg 뺐다가\\n요요로 다시\\n15kg 쪘습니다" (한 줄씩 15~18자 이내)
