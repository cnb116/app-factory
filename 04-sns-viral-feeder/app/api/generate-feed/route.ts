...
1. threadPost — 스레드(Threads)용 본문 원고 (Problem-Agitation-Solution 3단 구조 필수, 전 구간 1인칭 경험담 톤 유지)
...
- [Solution] 그다음 문장: 절대 광고 카피처럼 쓰지 않는다. "도구", "서비스", "앱", "이것", "이거 하나면" 같이 제품을 직접 가리키는 단어를 쓰지 말고, "그래서 답답한 마음에 뭘 어떻게 했더니 상황이 이렇게 편해졌다"는 순수 1인칭 후일담으로만 서술한다 (예: "그래서 답답한 마음에 직접 하나 만들어봤는데, 어제부터는 그 시간에 그냥 잡니더" 처럼 행동과 결과만 말하고 제품명·기능 설명은 하지 않는다). 제품을 알리는 역할은 바로 뒤에 붙는 "첫 댓글 링크"가 담당하므로, 본문에서는 제품을 설명하거나 홍보할 필요가 전혀 없다.
- 문장 사이사이 줄바꿈을 넉넉히 넣어(\\n\\n) 가독성 좋게 구성하고, 전체 3~4문장 분량을 유지한다.
- 해시태그를 문장 끝에 도배하듯 나열하지 않는다. 대신 핵심 키워드를 본문 문장 안에 자연스럽게 녹여쓴다.
- 본문 안에 URL/링크를 절대 넣지 않는다.
- 마지막 문장은 순수하게 공감·대화를 유도하는 질문으로 끝난다 (반드시 물음표로 마무리). "그래서 이게 필요하지 않으세요?"처럼 다시 제품 판매로 되돌아가는 문장은 금지 — "여러분은 오늘 밤 어떤 걸로 머리 싸매고 계십니까" 같이 순수하게 독자의 상황을 되묻는 톤만 허용한다.
